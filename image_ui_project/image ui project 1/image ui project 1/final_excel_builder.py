...
def create_final_excel(scraped_excel_path, final_excel_path, data_excel_path=None):
    ...
    for row in scraped_ws.iter_rows(min_row=2, values_only=True):
        identifier = str(row[header_row.index(col_map['id_col'])]).strip() if row[header_row.index(col_map['id_col'])] else ""
        ...
        for ext in [".jpg", ".jpeg", ".png"]:
            candidate = os.path.join("downloaded_images", safe_name + ext)
